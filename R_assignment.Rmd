...
Once again, the plot makes evident our initial observations. We can see that Saturday and Sunday (days 6 and 7) have far fewer articles (#1) and that Sunday has by far the lowest max shares (#2), as well as the smallest variance (#5).
...
#t.test(data_s$avg_shares[data_s$weekday_flag==1], data_s$avg_shares[data_s$weekday_flag==0], paired=TRUE)
```
The p-value is < 0.05 and less than the significance level of 5% so we do reject H0 and have statistically significant evidence that the number of shares differ between weekdays and weekends.
