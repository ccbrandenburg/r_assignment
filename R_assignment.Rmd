...
ggplot (aes(x=rate_positive_words, 
            y=shares), 
        data=data_s, 
        log="xy") +
...
qplot(data=data_s, 
      x=factor(data_channel), 
      y=1, 
      size=shares, 
      scale_y_discrete(0.5,1.5))
...
# set the seed to make results reproducible
set.seed(27)
data.subset <- data_s[, c(15, 16, 28, 29, 30, 31)]
ggpairs(data.subset[sample.int(nrow(data.subset), 500), ])
...
ggplot (data = data_s, aes(x=factor(data_channel), y = shares, group = as.integer(data_channel))) +
  geom_point()+
